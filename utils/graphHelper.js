...
// get all the user's file
function getAllUsersFile(accessToken, callback) {
  request
		.get('https://graph.microsoft.com/beta/me/drive/root/children?$select=id,name')
		// .send({ type: 'view' })
		.set('Authorization', 'Bearer ' + accessToken)
		.end((err, res) => {
			// Returns 200 OK and the permission with the link in the body.
...
exports.getAllUsersFile = getAllUsersFile;
