...
after(
	...
	function () {
		...
		// fs.writeFileSync(filePath, conf, { encoding: 'utf8' });
  return driver.quit();
}
);

test.describe('Automation', function () {
	// eslint-disable-line no-undef
  test.it('Connect Ignitetech Server Page', function (done) {
    driver.get('localhost:3000/server');
    driver.findElement(webdriver.By.id('connect_button')).click();
    driver.findElement(webdriver.By.name('loginfmt')).sendKeys(username);
    driver.findElement(webdriver.By.id('idSIButton9')).click();
    driver.findElement(webdriver.By.name('passwd')).sendKeys(password);
    driver.sleep(1000);
    let form = driver.findElement(webdriver.By.name('f1'));
    form.submit();
    driver.findElement(webdriver.By.id('idSIButton9')).click();
    driver.sleep(3000);
		// let form2 = driver.findElement(webdriver.By.tagName('form'));
		// form2.submit();
    driver.get('localhost:3000/server/status');
    driver.sleep(8000);
    driver.get('localhost:3000/server/stop');
		//     driver.wait(
		// 			function () {
		...
		// },
		// 			3000,
		// 			'Could not wait for the authentication page'
		// 		);
		...
		// },
		// 			3000,
		// 			'Could not show the send mail page'
		// 		);

		//     driver
		// 			.findElement(webdriver.By.css('.ms-font-m'))
