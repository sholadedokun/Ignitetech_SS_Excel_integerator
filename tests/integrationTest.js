...
var smartsheet = require('../utils/smartsheetHelper.js');
...
describe('Integration', function () {
  ...
  it(// eslint-disable-line no-undef
	'Checking that the sample can sendEmail', function (done) {
  var postBody = emailer.generateMailBody(
			'Olushola Adedokun',
			'olushola.adedokun@ignitetech.com'
		);
  graphHelper.postSendMail(accessToken, JSON.stringify(postBody), function (error) {
    assert(error === null, '\nThe sample failed to send an email');
    done();
  });
});
  it(// eslint-disable-line no-undef
    'Checking that can fetch from Smartsheet', function (done) {

      smartsheet.sendGetRequest('', 'https://api.smartsheet.com/2.0/sheets/', function (){
        done();
      });
